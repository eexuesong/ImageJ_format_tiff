classdef Simple_IFD
    ...
    properties
        NumDirEntries = uint16(15)
        
        NewSubFileType = uint32(0)
        ImageWidth = uint32(0)
        ImageLength = uint32(0)
        BitsPerSample = uint16(0)
        Compression = uint16(1)
        PhotometricInterpretation = uint16(1)

        % Image description part #2
        NumberCharsInImageDescription = uint32(0)
        OffsetOfImageDescription = uint32(0)
        ImageDescription

        StripOffsets = uint32(0)
        SamplesPerPixel = uint16(1)
        RowsPerStrip = uint16(0)
        StripByteCounts = uint32(0)
        XResolution = 0
        YResolution = 0
        ResolutionUnit = uint16(3)
        SampleFormat = uint16(1)
        
        % (ImageJ only)
        IJMetadataByteCounts
        IJMetadata

        % Image description part #1 (MicroManager only)
        NumberCharsInOMEXMLMetadata = uint32(0)
        OffsetOfOMEXMLMetadata = uint32(0)
        OMEXMLMetadata
    ...
    methods(Static)
        function TagList = tag_array(TagId, DataType, DataCount, DataOffset, Swapbytes_Flag)
            ...
            value_size_bytes = double(DataCount) * double(bytes_per_count);
